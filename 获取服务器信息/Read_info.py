...
class linux_info(object):
    ...
    def exce(self):
        ...
        # disk = len(psutil.disk_partitions())
        ...
        # netmask
        # netmask = psutil.net_if_addrs()['lo'][0][2]
...
class win_info(object):
    ...
    def main(self):
        ...
        cpuType = platform.uname()[5]

        # disk
        disk_num = 0
        disk_name = os.popen('wmic logicaldisk get size').read()
        for disk_list in disk_name.strip().replace('\n', '').split(' ')[1:]:
            if disk_list:
                disk_num += self.replace_str(disk_list)

        # disk = int(disk_num / 1000 / 1000 / 1000)
        # disk = psutil.disk_usage("/")[0] / (1024*1024*1024*1.0)
        # a = psutil.disk_partitions()
        # disk = 0
        # for i in range(len(a)):
        #     path = a[i][0]
